...
func walk(u interface{}, depth int) {
	val := reflect.Indirect(reflect.ValueOf(u)) // если указатель, разыменовывается
	t := val.Type()
	tabs := strings.Repeat("\t", depth+1) // depth помогает организовать структуры для форматирования вывода
	...
	if val.Kind() == reflect.Struct {
		for i := 0; i < t.NumField(); i++ {
			...
			if fieldVal.Kind() == reflect.Struct {
				walk(fieldVal.Interface(), depth+1) // if struct then make recursive call
			}
		}
	}
}
